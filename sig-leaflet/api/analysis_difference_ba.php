<?php
// api/analysis_difference_ba.php
...
try {
    ...
    $query_geom = "
        ...
        SELECT ST_AsGeoJSON(ST_Difference(geom_b, geom_a)) AS geojson
        FROM selected;
    ";
    ...
    // 2. Ambil marker yang berada di dalam area hasil selisih (B - A)
    $query_markers = "
        WITH selected AS (
            SELECT
                ST_Difference(b.geom, a.geom) AS hasil_geom,
                a.group_id
            FROM wilayah a
            JOIN wilayah b ON a.group_id = b.group_id
            WHERE a.id = :wilayah_a
    ...
    echo json_encode([
        'status' => 'error',
        'message' => 'Gagal melakukan analisis selisih B - A: ' . $e->getMessage()
    ]);
}
