...
try {
    ...
    $query_geom = "
        ...
        SELECT ST_AsGeoJSON(ST_Difference(geom_a, geom_b)) AS geojson
        FROM selected;
    ";
    ...
    // 2. Ambil marker yang berada di dalam area hasil selisih
    $query_markers = "
        WITH selected AS (
            SELECT
                ST_Difference(a.geom, b.geom) AS hasil_geom,
                a.group_id
            FROM wilayah a
            JOIN wilayah b ON a.group_id = b.group_id
            WHERE a.id = :wilayah_a
    ...
    echo json_encode([
        'status' => 'error',
        'message' => 'Gagal melakukan analisis selisih: ' . $e->getMessage()
    ]);
}
