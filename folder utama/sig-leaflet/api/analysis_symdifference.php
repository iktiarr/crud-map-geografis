<?php
// api/analysis_symdifference.php
...
try {
    // 1. Hitung symmetric difference wilayah (A △ B)
    $query_geom = "
        WITH selected AS (
            SELECT
                a.geom AS geom_a,
        ...
        SELECT ST_AsGeoJSON(ST_SymDifference(geom_a, geom_b)) AS geojson
        FROM selected;
    ";
    ...
    // 2. Ambil marker yang berada di dalam area hasil
    $query_markers = "
        WITH selected AS (
            SELECT
                ST_SymDifference(a.geom, b.geom) AS hasil_geom,
                a.group_id
            FROM wilayah a
            JOIN wilayah b ON a.group_id = b.group_id
            WHERE a.id = :wilayah_a
    ...
    echo json_encode([
        'status' => 'error',
        'message' => 'Gagal melakukan analisis symmetric difference: ' . $e->getMessage()
    ]);
}
